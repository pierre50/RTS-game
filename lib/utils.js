...
function changeSpriteColor(sprite, color){
	...
	let filters = []
	//let replacement
	//filters.push(new PIXI.filters.MultiColorReplaceFilter(source, colors[color], .1));
	for (let i = 0; i < colors[color].length; i ++){
		filters.push(new PIXI.filters.ColorReplaceFilter(source[i], colors[color][i], .1));
	}
...
function instanceContactInstance(a, b){
	return Math.floor(instancesDistance(a, b, true)) <= (b.size - 1 || 1);
}
	...
	while (!isFinish) {
		if(openCells.length > 0){
			...
			for(let i = 0; i < neighbours.length; i++){
				...
				if(!closedCells.includes(neighbour) && !neighbour.solid){
...
function renderCellOnInstanceSight(instance){
	getPlainCellsAroundPoint(instance.i, instance.j, instance.player.views, instance.sight, (cell) => {
		if (pointsDistance(instance.i, instance.j, cell.i, cell.j) <= instance.sight){
			...
			if (globalCell.has){
				if (globalCell.has.type === 'Tree' && instance.player.foundedTrees.indexOf(globalCell.has) === -1){
					instance.player.foundedTrees.push(globalCell.has);
				}
				if (globalCell.has.type === 'Berrybush' && instance.player.foundedBerrybushs.indexOf(globalCell.has) === -1){
					instance.player.foundedBerrybushs.push(globalCell.has);
				}
				//if (globalCell.has.player && globalCell.has.player !== instance.player){
	...
	getPlainCellsAroundPoint(instance.i, instance.j, instance.player.views, instance.sight, (cell) => {
		if (pointsDistance(instance.i, instance.j, cell.i, cell.j) <= instance.sight){
			...
			if (!cell.viewedBy.length){
				globalCell.setFog();
				if (globalCell.has){
					//if (globalCell.has.player && globalCell.has.player !== instance.player && globalCell.has.name === 'unit'){
