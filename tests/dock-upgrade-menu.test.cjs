...
test('Dock only shows the current boat in each upgrade chain', () => {
  ...
  assert.equal(config.units.FishingBoat.assets.actionSheet, '473')
  assert.equal(config.units.FishingBoat.assets.fishingOverlaySheet, '697')
  assert.equal(config.units.FishingShip.assets.actionSheet, '474')
  assert.equal(config.units.FishingShip.assets.fishingOverlaySheet, '700')
