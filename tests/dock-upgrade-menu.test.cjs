...
test('Dock only shows the current boat in each upgrade chain', () => {
  ...
  assert.equal(config.units.FishingBoat.assets.actionSheet, 'boats/fishing-boat/standing')
  assert.equal(config.units.FishingBoat.assets.fishingOverlaySheet, 'boats/fishing-boat/fishing-overlay')
  assert.equal(config.units.FishingShip.assets.actionSheet, 'boats/fishing-ship/body')
