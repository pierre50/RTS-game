...
test('topbar displays and themes all civilization ages', () => {
  withFakeDocument(() => {
    ...
    assert.equal(menu.gameHud.children.length, 1)
    assert.equal(menu.gameHud.children[0].className, 'topbar-options')
    assert.equal(menu.gameHud.children[0].children.length, 2)
    assert.equal(menu.gameHud.children[0].children[0].className, 'topbar-daytime hud-info-panel')
    assert.equal(menu.gameHud.children[0].children[0].textContent, 'Jour 3 - 06:15')
