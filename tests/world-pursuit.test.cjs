...
function collectionFixture() {
  const api = loadTsModule(require.resolve('../app/screens/game/GameWorldPursuers.ts'))
  ...
  const players = [
    { label: 'human', units: [hero, follower], isPlayed: true },
    { label: 'enemy', color: 'red', units: [soldier, dead, idle, interior] },
  ]
...
test('only living pursuers targeting a visible travelling party member cross the border', () => {
  ...
  assert.deepEqual(
    pursuers.map(entry => [entry.entity.label, entry.targetLabel]),
    [
      ['soldier', 'follower'],
      ['wolf', 'hero'],
    ]
...
test('departure save loses pursuers and occupied cell references, while the rollback snapshot stays intact', () => {
  ...
  assert.equal(
    departure.players[1].units.some(unit => unit.label === 'soldier'),
    false
  )
  assert.deepEqual(
    departure.animals.map(animal => animal.label),
    ['far']
  )
...
function runtimeFixture() {
  ...
  context.scheduler = new ActionScheduler(
    { ticker: { add() {}, remove: callback => removed.push(callback) } },
    () => context.paused
  )
  const { WorldPursuitSystem } = loadTsModule('app/services/world/WorldPursuitSystem.ts', {
    mocks: {
      ...
      './WorldRegionTravelSystem': {
        findOpenWorldTravelCell: (map, anchor) => (map.open ? { ...anchor, x: 32, y: 48, z: 0 } : null),
      },
    },
  })
  const entry = {
    entity: {
      ...
      hitPoints: 21,
      gender: 'female',
      appearanceVariants: { hair: 'black' },
      inventory: { equipment: ['sword'], equipped: { mainhand: 'sword' } },
      mountedOnHorse: true,
      horseColor: 'black',
      action: 'attack',
      path: [{ i: 49, j: 51 }],
...
test('pursuers arrive after three game seconds, preserving appearance, equipment, ownership and target', () => {
  ...
  for (const key of [
    ...
    assert.deepEqual(spawns[0][key], entry.entity[key])
