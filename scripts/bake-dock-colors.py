#!/usr/bin/env python3
"""
Bake dock player-color overlays into the final dock sprites.

For each civilization, composites the blue color overlay (dock/age-2-large)
onto the base sprite (dock/age-2) using anchor alignment, so
changeSpriteColorDirectly works directly on the final sprite.

Frames grow when the overlay overflows the base frame (greek age-3 mast);
...
def bake_civ(civ, dry_run=False):
    base_dir = GRAPHICS_DIR / civ / "dock/age-2"
    overlay_dir = GRAPHICS_DIR / civ / "dock/age-2-large"
