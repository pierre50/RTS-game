...
import { addHeroInventoryItem, getEquipmentSlot, getWeaponSlot } from '../../lib/equipment/equipmentLoot'
...
export function getAllHeroInventoryItems(): string[] {
  const specialItems = new Set<string>(SPECIAL_HERO_INVENTORY_ITEMS)
...
function getHeroWithInventory(context: DevConsoleContext): NonNullable<ReturnType<typeof getHero>> | null {
  const hero = getHero(context)
  if (!hero) return null
  ensureHeroInventory(context)
  return hero
}
...
export function addHeroInventoryEquipment(
  ...
  const hero = getHeroWithInventory(context)
  if (!hero) return { ok: false, message: 'No hero found' }
