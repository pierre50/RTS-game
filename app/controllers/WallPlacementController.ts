...
export class WallPlacementController {
  ...
  canUseCell: (cell: RuntimeCell, owner: PlacementOwner, allowExistingWall?: boolean) => boolean
  onCommit: (path: RuntimeCell[], owner: PlacementOwner) => boolean | void
  onChange: (() => void) | null
  ...
  handleClick(cell: RuntimeCell, owner: PlacementOwner): boolean {
  ...
  render(): void {
    ...
    draft.path.forEach((cell, index) => {
      ...
      if (getWallLevel(draft.owner) === 1 && wallFrame === 2) {
        const spritesheet = Assets.cache.get(WALL_CONSTRUCTION_FLAG_SHEET_ID) as { textures: Record<string, Texture> }
