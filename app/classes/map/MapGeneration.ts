...
type MapGenerationMap = RuntimeMap &
  UnknownRecord & {
    context: UnknownRecord & {
      menu?: UnknownRecord
      controls?: UnknownRecord
      players?: PlayerLike[]
      player?: PlayerLike
      scheduler?: { elapsedMs: number }
      performance?: { record?: (name: string, duration: number) => void }
    }
...
type SavedPlayer = UnknownRecord & {
  ...
  aiState?: UnknownRecord
}
type SavedGameData = {
  map: Array<Array<UnknownRecord & { type: string; z?: number; fogSprites?: unknown[] }>>
  players: SavedPlayer[]
