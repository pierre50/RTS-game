...
type BirdPath = { degree: number; end: Point; start: Point }
type AmbientContext = {
  app: {
    ticker: {
      add: (callback: (ticker: { deltaMS?: number; deltaTime?: number }) => void) => void
      remove: (callback: (ticker: { deltaMS?: number; deltaTime?: number }) => void) => void
    }
  }
  defeat?: boolean
  paused?: boolean
...
function randomBetween(min: number, max: number, random: RandomFn = Math.random): number {
  return min + random() * (max - min)
}

function sortedTextureNames(textures: Record<string, Texture>): string[] {
  return Object.keys(textures).sort((a, b) => parseInt(a, 10) - parseInt(b, 10))
}

export function getBirdDirectionalFrames(textures: Record<string, Texture>, degree: number, frameCount: number) {
...
export function createBirdPath(bounds: Bounds, random: RandomFn = Math.random): BirdPath {
...
export class AmbientBirds extends Container {
  activePasses: ActiveBirdPass[]
  context: AmbientContext
  elapsedMs: number
  getViewportBounds: () => Bounds
  nextPassAt: number
  random: RandomFn
  _onTick: (ticker: { deltaMS?: number; deltaTime?: number }) => void

  constructor(context: AmbientContext, getViewportBounds: () => Bounds, random: RandomFn = Math.random) {
    super()
  ...
  createAnimatedSprite(sheetId: string, degree: number, frameCount: number): AnimatedSprite | null {
  ...
  finishPass(pass: ActiveBirdPass): void {
    pass.group.destroy({ children: true })
    this.activePasses = this.activePasses.filter((p: ActiveBirdPass) => p !== pass)
  }

  update(elapsedMs: number, deltaTime: number): void {
