class Unit extends PIXI.Container {
	constructor(i, j, map, player, options = {}){
		...
		this.intervale = setInterval(() => this.step(), 1000);
	...
	setPath(path){
		...
		this.path = path;
		this.intervale = setInterval(() => this.step(), 16.66);
	}
	sendTo(dest, action){
		this.stop();
		let path = [];
	...
	getAction(name){
		...
		switch (name){
			...
			case 'deliveryberry':
				...
				break;
			case 'build':
				if (!this.dest || this.dest.isBuilt || this.dest.life >= this.dest.lifeMax){
					this.affectNewDest();
					return;
				}
				sprite.onLoop = () => {
	...
	affectNewDest(){
		...
		switch (this.work){
			...
			case 'gatherer':
				...
				break;
			case 'builder':
				condition = (instance) => (instance.name === 'building' && (!instance.isBuilt || instance.life < instance.lifeMax) && instance.player === this.player);
				break;
			case 'attacker':
	...
	isAttacked(instance){
		//this.runaway();
		//return;
		if (!instance || this.action === 'attack'){
			return;
		}
